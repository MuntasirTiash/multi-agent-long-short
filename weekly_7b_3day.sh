...
#SBATCH --output=results/weekly_7b_%j.log

# ===========================================================================
# WEEKLY walk-forward topology comparison — 498 firms, Qwen2.5-7B, transformers.
#   run_llm_analysis.py: no-comm (round 0) baseline vs each communication
#   topology, graded with rank-IC, transaction costs and a Monte-Carlo null.
#   Round 0 is computed ONCE per date and replayed for every topology, so the
#   baseline and the treatments see identical analyst opinions.
#
...
#   32 of the 74 weekly rebalances covers 2024-12-31 .. roughly Aug-2025. The
#   full 74 weeks needs ~5.2 days at this rate, so it needs vLLM's continuous
#   batching rather than a longer wall clock. (Alternatively a single topology
#   fits all 74 weeks: TOPOLOGIES=sparse.)
#
# A timeout would lose llm_analysis_*.json (written after the loop), so the date
# count is deliberately conservative. The transcript JSONL is written
# incrementally, so even a killed job leaves the per-agent record behind.
#
# Submit from the agent_orchestration directory:  sbatch weekly_7b_3day.sh
#   sbatch --export=ALL,MAX_DATES=10 weekly_7b_3day.sh        # shorter probe
#   sbatch --export=ALL,TOPOLOGIES=sparse weekly_7b_3day.sh   # all 74 weeks
# ===========================================================================
...
echo "==== [2/3] Weekly walk-forward (model=$LLM_MODEL, dates=$MAX_DATES, topo=$TOPOLOGIES)"
