...
#SBATCH --output=results/daily_vllm_%j.log

# ===========================================================================
# DAILY manager-agent backtest, 498 firms, Qwen2.5-7B served by vLLM.
#   run_daily_backtest.py -> per-topology Sharpe + the flexible manager book
#
# Every rebalance is a trading day, so this is the expensive arm:
#   373 dates x (498 round-0 + 3 topologies x 2 rounds x 498) ~= 557k LLM calls
# The job prints measured tokens/second per round and a projection to the full
# date range (instrumentation.py), so if throughput is too low to finish, that
# is visible in the log within the first few dates rather than at the deadline.
#
# Submit from the agent_orchestration directory:  sbatch daily_vllm_3day.sh
# Scale at submit time, e.g.:
#   sbatch --export=ALL,MAX_DATES=40 daily_vllm_3day.sh
#   sbatch --export=ALL,TOPOLOGIES=sparse daily_vllm_3day.sh
# ===========================================================================
...
python -m vllm.entrypoints.openai.api_server \
    ...
    > results/daily_vllm_server_${SLURM_JOB_ID}.log 2>&1 &
VLLM_PID=$!
cleanup() { [ -n "${VLLM_PID:-}" ] && kill "$VLLM_PID" 2>/dev/null || true; }
trap cleanup EXIT
echo "vLLM pid=$VLLM_PID (server log: results/daily_vllm_server_${SLURM_JOB_ID}.log)"
...
for i in $(seq 1 180); do
    ...
    if ! kill -0 "$VLLM_PID" 2>/dev/null; then
        ...
        tail -40 results/daily_vllm_server_${SLURM_JOB_ID}.log; exit 1
    fi
    sleep 5
done
...
echo "==== [4/5] Daily backtest (topologies=$TOPOLOGIES) ======================="
...
ls -td results/daily_* 2>/dev/null | head -1
exit $STATUS
